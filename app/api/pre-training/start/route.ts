...
export async function POST(request: Request) {
  ...
  const body = await readJsonObject(request) as { sessionPlan?: unknown } | null;
  if (!body) return productError("INVALID_REQUEST", "Tell FightIQ what you are training first.", 400);
  const sessionPlan = typeof body.sessionPlan === "string" ? cleanText(body.sessionPlan).replace(/\s+/g, " ").trim() : "";
