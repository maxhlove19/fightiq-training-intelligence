...
export function summariseHealth(checks: HealthChecks): HealthReport {
  ...
  if (!checks.sessionAnalysis) notes.push("No ANTHROPIC_API_KEY. Sessions still save and are kept in full; the debrief and Coach say the reading half is not switched on. Adding the key makes past sessions readable, and nothing is lost in the meantime.");
